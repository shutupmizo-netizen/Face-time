...
# Owner config - I duh chuan thlak rawh
...
def handle_signup(data):
    ...
    if username in users_db:
        emit('auth_error', {'msg': 'Username hman a ni tawh. Dang thlang rawh / Username already taken'})
        return

    if len(username) < 3:
        emit('auth_error', {'msg': 'Username 3 characters ai a tawi thei lo / Username too short'})
        return

    if len(password) < 4:
        emit('auth_error', {'msg': 'Password 4 characters ai a tawi thei lo / Password too short'})
        return
...
def handle_login(data):
    ...
    if username not in users_db:
        emit('auth_error', {'msg': 'Account a awm lo. Sign Up phawt rawh / Account not found'})
        return

    stored_hash = users_db[username]['password_hash']
    if not bcrypt.checkpw(password.encode('utf-8'), stored_hash):
        emit('auth_error', {'msg': 'Password dik lo / Wrong password'})
        return
...
def verify_session(data):
    ...
    if session_id in sessions_db:
        ...
        emit('auth_error', {'msg': 'Session expired. Login leh rawh'})
...
def handle_vote(data):
    ...
    if users_db[username]['coins'] < 10:
        emit('vote_error', {'msg': 'Coins a tlem. Lei rawh'})
        return
...
def handle_chat(data):
    ...
    for word in bad_words:
        if word in msg_lower:
            emit('chat_error', {'msg': 'Thu mawi lo hman phal loh / Bad words not allowed'})
            return
...
def reset_votes(data):
    ...
    game_data['enemy'] = 0
    emit('update', game_data, broadcast=True)
    add_system_chat("🔄 Votes reset a ni e / Votes have been reset")
...
def start_stream(data):
    ...
    add_system_chat(f"🔴 {username} a rawn LIVE e! / {username} is now LIVE!")
...
def stop_stream(data):
    ...
    add_system_chat(f"⏹️ {username} live a tawp e / {username} ended stream")
